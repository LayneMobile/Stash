...
import static stash.Constants.SourceModule;
...
final class ApiBuilder {
    ...
    ApiBuilder(TypeElement api, Types typeUtils) {
        ...
        StashLog.d(TAG, "superTypeParams: %s", superTypeParams);
...
//        javaFile.writeTo(System.out);
    ...
    private JavaFile build() {
        ...
        StashLog.d(TAG, "GenerateApiBuilder element: %s", api.getQualifiedName());
        GenerateApiBuilder annotation = api.getAnnotation(GenerateApiBuilder.class);
        try {
            Class<? extends SourceModuleBuilder>[] builders = annotation.value();
